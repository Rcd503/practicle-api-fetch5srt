...
import Constants from 'expo-constants';
 ...
 class App extends React.Component {
  ...
  //console.warn(respJson.data)
   ...
   this.setState({data1:respJson.data})
    }  
    
    rennderitemview(item){
      console.log(item);
    <View>
      <Text style={{fontSize:25}}>{item.data}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={{fontSize:35}}> Mob Practicle</Text>
     
      <FlatList
      data={this.state.data1}
      
      renderItem={
      ({item}) => {this.rennderitemview(item)}
